...
public data class GeminiStreamingApiProvider(
...
) : StreamingApiProvider<Content, GenerateContentResponse> {
    ...
    override suspend fun invoke(p1: History<Content>): Flow<GenerateContentResponse> {
        val systemInstruction = p1.firstOrNull()?.takeIf { it.role == "system" }
        val contents = if (systemInstruction != null) p1.drop(1) else p1
        return controller.generateContentStream(
